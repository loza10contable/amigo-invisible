...
def send_emails_background(assignments, base_url):
    for a in assignments:
        try:
            ...
            send_email(
                ...
                <p><a href="{link}">👉 Descubrir acá</a></p>
                """
            )
        except Exception as e:
...
def admin_send():
    ...
    # ✅ THREAD BACKGROUND (CLAVE)
    t = threading.Thread(
        target=send_emails_background,
        args=(assignments, base),
        daemon=True
    )
    t.start()

    flash(
        "✅ Envío de emails iniciado en segundo plano.",
        "success"
    )
    return redirect(url_for("admin_panel", key=ADMIN_KEY))
